...
# My Ledger — Product Requirements Document
...
For MVP, the bookkeeper is **one person — solo ledger ownership**. A future iteration extends the product to two partners sharing a household ledger.
...
- A transaction the user has deleted remains recoverable through a user-facing surface for at least a short window after the delete commits; the product does not permanently remove a transaction on the first user action.
...
The product defines two roles from day one:
...
**MVP scope: only the Owner role is reachable.** There is no invitation flow, no second user on any ledger, and no UI to assign a Viewer. The Viewer role exists as groundwork for the v2+ couple-sharing direction.
...
- **Multi-user collaboration on a single ledger.** Each ledger has exactly one Owner in MVP; the Viewer role is not reachable via UI. (Phase 2 decision.)
...
2. **What is the undo window for deleted transactions, and through what surface does the user recover?** — Owner: user. The NFR commits to "at least a short window" of recoverability and forbids permanent removal on first action, but does not pin the duration (a transient undo banner valid for ~30 seconds? a "recently deleted" view with a longer retention?). Resolve before or during MVP build.
